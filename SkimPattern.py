...
class SkimmerPattern:
    def __init__(self, pattern_file="junk.lst", master="nonMASTER.SCP"):
        self.calls = {}
        self.skimmer_data = {}
    ...
    def check_skimmer_rules(self):
        ...
        print(
            f"rule match percentage is {(len(self.skimmer_data) -unmatch_rule_count) * 100 /len(self.skimmer_data):5.3f} %"
        )
    ...
    def check_master_rules(self):
        ...
        print(
            f"call match percentage is {(len(self.calls)-unmatched_count) * 100 / len(self.calls):5.3f}%"
        )
